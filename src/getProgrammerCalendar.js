...
module.exports = () => {
    ...
    let drinks = ["水","茶","红茶","绿茶","咖啡","奶茶","可乐","鲜奶","豆奶","果汁","果味汽水","苏打水","运动饮料","酸奶","酒"];

    let result = {
        date: getTodayString(today,weeks),
        good: [],
        bad: [],
        directions: directions[random(iday, 2) % directions.length],
        todayDrink: pickRandom(drinks,2,iday).join('，')
    }

    pickTodaysLuck(result,today,varNames,tools,activities,iday,specials);
...
function pickTodaysLuck(result, today, varNames, tools, activities,iday,specials) {
    ...
    let eventArr = pickRandomActivity(_activities, numGood + numBad, varNames, tools);

    let specialSize = pickSpecials(specials,iday);

    for (let i = 0; i < numGood; i++) {
        addToGood(eventArr[i],result);
    }

    for (let i = 0; i < numBad; i++) {
        addToBad(eventArr[numGood + i],result);
    }
}
function pickRandomActivity(activities, size, varNames, tools) {
	var picked_events = pickRandom(activities, size);	
	for (var i = 0; i < picked_events.length; i++) {
		picked_events[i] = parse(picked_events[i],varNames,tools);
	}
	return picked_events;
}
// 添加预定义事件
function pickSpecials(specials,iday) {
	var specialSize = [0,0];	
...
// 去掉一些不合今日的事件
function filter(activities,today) {
    let result = [];
...
// 添加到“宜”
function addToGood(event,result) {
	result.good.push(event);
}
// 添加到“不宜”
function addToBad(event,result) {
	result.bad.push(event);
}
// 解析占位符
function parse(event,varNames,tools) {
